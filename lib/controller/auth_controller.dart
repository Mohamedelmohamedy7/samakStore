...
class AuthController extends GetxController implements GetxService {
  ...
  String net_by_searchData="";
  bool isLoadingSearchData = false;
  Future<void> netBySearch({String time}) async {
    print(time);
    isLoadingSearchData = true;
    update();
    Response response = await authRepo.netBySearch(time:time);
    log("${response.body}*/*/*/**");
    if (response.statusCode == 200) {
      isLoadingSearchData = false;
      net_by_searchData =response.body["data"].toString();
      update();
    } else {
      isLoadingSearchData = false;
      update();
